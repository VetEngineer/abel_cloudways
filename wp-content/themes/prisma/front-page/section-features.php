<div class="front_page_section front_page_section_features<?php
	$prisma_scheme = prisma_get_theme_option( 'front_page_features_scheme' );
	if ( ! empty( $prisma_scheme ) && ! prisma_is_inherit( $prisma_scheme ) ) {
		echo ' scheme_' . esc_attr( $prisma_scheme );
	}
	echo ' front_page_section_paddings_' . esc_attr( prisma_get_theme_option( 'front_page_features_paddings' ) );
	if ( prisma_get_theme_option( 'front_page_features_stack' ) ) {
		echo ' sc_stack_section_on';
	}
?>"
		<?php
		$prisma_css      = '';
		$prisma_bg_image = prisma_get_theme_option( 'front_page_features_bg_image' );
...
<?php
	// Add anchor
	$prisma_anchor_icon = prisma_get_theme_option( 'front_page_features_anchor_icon' );
	$prisma_anchor_text = prisma_get_theme_option( 'front_page_features_anchor_text' );
...
?>
	...
	<?php
	if ( prisma_get_theme_option( 'front_page_features_fullheight' ) ) {
	...
	?>
			...
			<?php
			$prisma_css      = '';
			$prisma_bg_mask  = prisma_get_theme_option( 'front_page_features_bg_mask' );
			$prisma_bg_color_type = prisma_get_theme_option( 'front_page_features_bg_color_type' );
			if ( 'custom' == $prisma_bg_color_type ) {
				$prisma_bg_color = prisma_get_theme_option( 'front_page_features_bg_color' );
	...
	>
		<div class="front_page_section_content_wrap front_page_section_features_content_wrap content_wrap">
			<?php
			// Caption
			$prisma_caption = prisma_get_theme_option( 'front_page_features_caption' );
			if ( ! empty( $prisma_caption ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<h2 class="front_page_section_caption front_page_section_features_caption front_page_block_<?php echo ! empty( $prisma_caption ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( $prisma_caption, 'prisma_kses_content' ); ?></h2>
				<?php
			}

			// Description (text)
			$prisma_description = prisma_get_theme_option( 'front_page_features_description' );
			if ( ! empty( $prisma_description ) || ( current_user_can( 'edit_theme_options' ) && is_customize_preview() ) ) {
				?>
				<div class="front_page_section_description front_page_section_features_description front_page_block_<?php echo ! empty( $prisma_description ) ? 'filled' : 'empty'; ?>"><?php echo wp_kses( wpautop( $prisma_description ), 'prisma_kses_content' ); ?></div>
				<?php
			}
			...
			<div class="front_page_section_output front_page_section_features_output">
				...
				if ( is_active_sidebar( 'front_page_features_widgets' ) ) {
					...
					if ( ! prisma_exists_trx_addons() ) {
						...
						prisma_customizer_need_widgets_message( 'front_page_features_caption', 'ThemeREX Addons - Services' );
					}
				}
				?>
			</div>
