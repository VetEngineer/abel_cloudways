...
jQuery( window ).on( 'load', function() {
	...
	if ( $editor_wrapper.length ) {
		...
		if ( typeof window.MutationObserver !== 'undefined' ) {
			...
			prisma_create_observer( 'prisma-check-visual-editor-wrapper', $editor_wrapper, function( mutationsList ) {
				prisma_gutenberg_editor_init();
			} );
		} else {
		 	prisma_gutenberg_editor_init();
		}
	...
	function prisma_gutenberg_editor_object() {
		var editor = {
			$editor: false,
			$frame: false,
			...
			if ( $editor.length ) {
				...
				if ( editor.$frame.length ) {
					if ( editor.$frame.get(0).contentDocument ) {
						...
						if ( editor.$styles_wrapper.hasClass( 'prisma_inited' ) ) {
					...
					if ( ! editor.$editor.hasClass( 'prisma_inited' ) ) {
	...
	// Init on page load
	function prisma_gutenberg_editor_init() {

		// Get Gutenberg editor object
		var editor = prisma_gutenberg_editor_object();
		if ( ! editor.$editor ) {
			return;
		}
		...
		function add_class_with_color_scheme( $obj ) {
			if ( ! $obj.hasClass( 'scheme_' + PRISMA_STORAGE['color_scheme'] ) ) {
				$obj.addClass( 'scheme_' + PRISMA_STORAGE['color_scheme'] );
			}
		}

		function add_class_with_overridden_options( $obj ) {
			for ( var i in PRISMA_STORAGE['override_classes'] ) {
				$obj.addClass( PRISMA_STORAGE['override_classes'][i].replace( '%s', PRISMA_STORAGE[ i ] ) );
			}
		}
		...
		if ( editor_type == 'post' && editor.$styles_wrapper ) {
			...
			editor.$styles_wrapper.addClass( prisma_get_class_by_prefix( editor.$editor.attr( 'class' ), 'post-type-' ) );
			...
			if ( editor.$writing_flow && PRISMA_STORAGE['sidebar_position'] != 'hide' ) {
				if ( prisma_apply_filters( 'prisma_filter_wrap_gutenberg_editor_with_sidebar', false ) ) {
		...
		if ( editor_type == 'site' && editor.$styles_wrapper ) {
			...
			add_class_with_overridden_options( editor.$styles_wrapper );
			// Mark as inited
			editor.$styles_wrapper.addClass( 'prisma_inited' );
		}
		...
		if ( editor_type == 'widgets' && editor.$writing_flow ) {
			...
			if ( prisma_apply_filters( 'prisma_filter_add_scheme_to_widgets_editor', true ) ) {
				...
				prisma_remove_observer( 'prisma-check-editor-styles-wrapper' );
				prisma_create_observer( 'prisma-check-editor-styles-wrapper', editor.$writing_flow, function( mutationsList ) {
		...
		editor.$editor.addClass( 'prisma_inited' );
	}
} );
