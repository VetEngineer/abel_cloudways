...
jQuery( document ).ready(
	function() {
		...
		prisma_add_filter('trx_addons_filter_sc_classes', function(classes, settings) {
		...
		setTimeout( function() {
			if ( window.elementor !== undefined ) {
				...
				jQuery( '#elementor-panel' )
					.on( 'input change', '[data-setting^="prisma_options_"]', function (e) {
						...
						var link = jQuery( this ).parents( '.elementor-control' ).find( 'a.prisma_post_editor' );
						if ( link.length > 0 ) {
							prisma_change_post_edit_link_elementor( link );
						}
					} )
					.on( 'click', '.prisma_post_editor', function(e) {
						prisma_change_post_edit_link_elementor( jQuery(this) );
						if (jQuery(this).hasClass('prisma_hidden' )) {
							e.preventDefault();
							return false;
						}
					});
		...
		function prisma_change_post_edit_link_elementor(a) {
			if (a.length > 0) {
