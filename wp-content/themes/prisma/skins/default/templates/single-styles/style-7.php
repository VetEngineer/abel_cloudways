...
if ( apply_filters( 'prisma_filter_single_post_header', is_singular( 'post' ) || is_singular( 'attachment' ) ) ) {
    $prisma_post_format = str_replace( 'post-format-', '', get_post_format() );
    $post_meta = in_array( $prisma_post_format, array( 'video' ) ) ? get_post_meta( get_the_ID(), 'trx_addons_options', true ) : false;
	...
	ob_end_clean();
	$prisma_with_featured_image = prisma_is_with_featured_image( $prisma_post_header /*, array( 'with_gallery' )*/);
	// Post title and meta
	ob_start();
	...
	if ( strpos( $prisma_post_header, 'post_featured' ) !== false
		...
		<div class="post_header_wrap post_header_wrap_in_header post_header_wrap_style_<?php
            ...
            if ( $prisma_with_featured_image ) {
                echo ' with_featured_image' . ( false && prisma_get_theme_option( 'single_parallax' ) == 0 ? ' prisma-full-height' : '' );
            }
		?>">
			<?php
