...
if ( ! function_exists( 'prisma_array_get_first' ) ) {
	/**
	 ...
	 */
	function prisma_array_get_first( &$arr, $key = true ) {
		foreach ( $arr as $k => $v ) {
			break;
		}
		return $key ? $k : $v;
...
if ( ! function_exists( 'prisma_array_get_keys_by_value' ) ) {
	/**
	 ...
	 */
	function prisma_array_get_keys_by_value( $arr, $value = 1 ) {
		if ( ! is_array( $arr ) ) {
...
if ( ! function_exists( 'prisma_array_delete_by_value' ) ) {
	/**
	 ...
	 */
	function prisma_array_delete_by_value( $arr, $value ) {
...
if ( ! function_exists( 'prisma_array_from_list' ) ) {
	/**
	 ...
	 */
	function prisma_array_from_list( $arr ) {
		$new = array();
		foreach ( $arr as $v ) {
			$new[ $v ] = $v;
		}
...
if ( ! function_exists( 'prisma_array_slice' ) ) {
	/**
	 ...
	 */
	function prisma_array_slice( $arr, $from, $to='' ) {
...
if ( ! function_exists( 'prisma_array_merge' ) ) {
	/**
	 * Merge arrays and lists (preserve a numeric indexes).
	 *
	 * For example:
	 *
	 * prisma_array_merge( [ 1, 2, 3, 4 ], [ 'one', 'two' ] )  // result: [ 'one', 'two', 3, 4 ]
	 *
	 * prisma_array_merge( [ 'a' => 'A', 'b' => 'B' ], [ 'one', 'two', 'a' => 'AA' ] )  // result: [ 'a' => 'AA', 'b' => 'B', 0 => 'one', 1 => 'two' ]
	 *
	 ...
	 */
	function prisma_array_merge( $a1, $a2 ) {
...
if ( ! function_exists( 'prisma_array_insert_after' ) ) {
	/**
	 ...
	 */
	function prisma_array_insert_after( &$haystack, $needle, $stuff ) {
		...
		for ( $i = 2; $i < func_num_args(); ++$i ) {
			...
			if ( is_array( $arg ) ) {
				if ( 2 == $i ) {
					...
					$new_array = prisma_array_merge( $new_array, $arg );
				}
			} else {
				$new_array[] = $arg;
			}
		...
		$haystack = is_int( $needle )
						...
						: prisma_array_merge( array_slice( $haystack, 0, $i, true ), $new_array, array_slice( $haystack, $i, null, true ) );

		return $i;
	}
}

if ( ! function_exists( 'prisma_array_insert_before' ) ) {
	/**
	 * Inserts any number of scalars or arrays at the point in the haystack
	 ...
	 * @return int               The index at which $needle was found.
	 */
	function prisma_array_insert_before( &$haystack, $needle, $stuff ) {
		if ( ! is_array( $haystack ) ) {
			return -1;
		}
...
if ( ! function_exists( 'prisma_array_get_by_key' ) ) {
	/**
	 ...
	 */
	function prisma_array_get_by_key( &$arr, $key, $value, $key2 = '', $default = '' ) {
		$rez = $default;
		foreach ( $arr as $v ) {
...
if ( ! function_exists( 'prisma_array_str_replace' ) ) {
	/**
	 ...
	 */
	function prisma_array_str_replace( $from, $to, $key, $arr ) {
		foreach ( $arr as $k => $v ) {
			if ( is_array( $v ) ) {
				$arr[ $k ] = prisma_array_str_replace( $from, $to, $key, $v );
			} else if ( $k == $key ) {
...
if ( ! function_exists( 'prisma_generate_id' ) ) {
	/**
	 ...
	 */
	function prisma_generate_id( $prefix = '' ) {
		return $prefix . str_replace( '.', '', mt_rand() );
	}
}

if ( ! function_exists( 'prisma_get_tag' ) ) {
	/**
	 ...
	 */
	function prisma_get_tag( $text, $tag_start, $tag_end = '' ) {
		$val       = '';
...
if ( ! function_exists( 'prisma_get_tag_attrib' ) ) {
	/**
	 ...
	 */
	function prisma_get_tag_attrib( $text, $tag, $attr ) {
		$val       = '';
...
if ( ! function_exists( 'prisma_get_css_position_from_values' ) ) {
	/**
	 ...
	 */
	function prisma_get_css_position_from_values( $top = '', $right = '', $bottom = '', $left = '', $width = '', $height = '' ) {
		if ( ! is_array( $top ) ) {
		...
		foreach ( $top as $k => $v ) {
			...
			if ( '' != $v ) {
				$output .= ( 'width' == $k ? 'width' : ( 'height' == $k ? 'height' : 'margin-' . esc_attr( $k ) ) ) . ':' . esc_attr( prisma_prepare_css_value( $v ) ) . ( '!' == $imp ? ' !important' : '' ) . ';';
			}
		}
		return $output;
	}
...
if ( ! function_exists( 'prisma_prepare_css_value' ) ) {
	/**
	 ...
	 */
	function prisma_prepare_css_value( $val ) {
		if ( '' !== $val ) {
...
if ( ! function_exists( 'prisma_parse_icons_classes' ) ) {
	/**
	 ...
	 */
	function prisma_parse_icons_classes( $css ) {
		$rez = array();
		if ( ! file_exists( $css ) ) {
			return $rez;
		}
...
if ( ! function_exists( 'prisma_get_column_class' ) ) {
	/**
	 ...
	 */
	function prisma_get_column_class( $num, $all, $all_tablet = '', $all_mobile = '' ) {
...
if ( ! function_exists( 'prisma_get_svg_from_file' ) ) {
	...
	function prisma_get_svg_from_file( $svg ) {
		if ( prisma_is_url( $svg ) && ! prisma_is_external_url( $svg ) ) {
			$svg_path = prisma_url_to_local_path( $svg );
			if ( ! empty( $svg_path ) ) {
				$svg = $svg_path;
			}
		}
...
if ( ! function_exists( 'prisma_external_links_target' ) ) {
	/**
	 ...
	 */
	function prisma_external_links_target( $admin = false ) {
...
if ( ! function_exists( 'prisma_stripslashes' ) ) {
	/**
	 ...
	 */
	function prisma_stripslashes( $val ) {
		static $magic = 0;
		if ( 0 === $magic ) {
...
if ( ! function_exists( 'prisma_get_value_gp' ) ) {
	/**
	 ...
	 */
	function prisma_get_value_gp( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
...
if ( ! function_exists( 'prisma_get_value_gpc' ) ) {
	/**
	 ...
	 */
	function prisma_get_value_gpc( $name, $defa = '' ) {
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
			$rez = wp_unslash( $_POST[ $name ] );
...
if ( ! function_exists( 'prisma_get_value_gps' ) ) {
	/**
	 ...
	 */
	function prisma_get_value_gps( $name, $defa = '' ) {
		global $wp_session;
		if ( isset( $_GET[ $name ] ) ) {
			$rez = wp_unslash( $_GET[ $name ] );
		} elseif ( isset( $_POST[ $name ] ) ) {
...
if ( ! function_exists( 'prisma_get_session_value' ) ) {
	/**
	 ...
	 */
	function prisma_get_session_value( $name, $defa = '' ) {
		global $wp_session;
		return isset( $wp_session[ $name ] ) ? $wp_session[ $name ] : $defa;
	}
}

if ( ! function_exists( 'prisma_set_session_value' ) ) {
	/**
	 ...
	 */
	function prisma_set_session_value( $name, $value ) {
		global $wp_session;
		$wp_session[ $name ] = $value;
	}
}

if ( ! function_exists( 'prisma_del_session_value' ) ) {
	/**
	 * Delete a saved variable from the current session.
	 *
	 * @param string $name   A name to to delete from the session.
	 */
	function prisma_del_session_value( $name ) {
		global $wp_session;
		unset( $wp_session[ $name ] );
	}
}

if ( ! function_exists( 'prisma_get_cookie' ) ) {
	/**
	 ...
	 */
	function prisma_get_cookie( $name, $defa = '' ) {
		return prisma_stripslashes( isset( $_COOKIE[ $name ] ) ? $_COOKIE[ $name ] : $defa );
	}
}

// Set a cookie - wrapper for setcookie using WP constants.
if ( ! function_exists( 'prisma_set_cookie' ) ) {
	/**
	 ...
	 */
	function prisma_set_cookie( $name, $value, $expire = 0, $secure = false, $httponly = false  ) {
		if ( ! headers_sent() ) {
			if ( defined( 'PHP_VERSION_ID' ) && PHP_VERSION_ID >= 70300 ) {
				$rez = setcookie(
							$name,
							$value,
							apply_filters( 'prisma_filter_cookie_options', array(
								'expires'  => $expire,
								'path'     => defined( 'COOKIEPATH' ) && ! empty( COOKIEPATH ) ? COOKIEPATH : '/',
								'domain'   => defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//prisma_get_domain_from_url( home_url() ),
								'secure'   => $secure,
								'httponly' => $httponly,
				...
				$rez = setcookie(
							...
							defined( 'COOKIE_DOMAIN' ) && ! empty( COOKIE_DOMAIN ) ? COOKIE_DOMAIN : '',	//prisma_get_domain_from_url( home_url() ),
							$secure,
							$httponly
						);
			}
			...
			trigger_error( sprintf( __( '%1$s cookie cannot be set - headers already sent by %2$s on line %3$s', 'prisma' ), $name, $file, $line ), E_USER_NOTICE ); // @codingStandardsIgnoreLine
		}
	}
}
...
if ( ! function_exists( 'prisma_hex2rgb' ) ) {
	/**
...
if ( ! function_exists( 'prisma_hex2rgba' ) ) {
	/**
	 ...
	 */
	function prisma_hex2rgba( $hex, $alpha ) {
		$rgb = prisma_hex2rgb( $hex );
		return 'rgba(' . intval( $rgb['r'] ) . ',' . intval( $rgb['g'] ) . ',' . intval( $rgb['b'] ) . ',' . floatval( $alpha ) . ')';
	}
}

if ( ! function_exists( 'prisma_rgba2hex' ) ) {
	/**
	 ...
	 */
	function prisma_rgba2hex( $color, $alpha = false ) {
...
if ( ! function_exists( 'prisma_hex2hsb' ) ) {
	/**
	 ...
	 */
	function prisma_hex2hsb( $hex, $h = 0, $s = 0, $b = 0 ) {
		$hsb      = prisma_rgb2hsb( prisma_hex2rgb( $hex ) );
...
if ( ! function_exists( 'prisma_rgb2hsb' ) ) {
	/**
...
if ( ! function_exists( 'prisma_hsb2rgb' ) ) {
	/**
...
if ( ! function_exists( 'prisma_rgb2hex' ) ) {
	/**
...
if ( ! function_exists( 'prisma_hsb2hex' ) ) {
	/**
	 ...
	 * @return string     A string with a hex color presentation in the format '#RRGGBB'.
	 */
	function prisma_hsb2hex( $hsb ) {
		return prisma_rgb2hex( prisma_hsb2rgb( $hsb ) );
	}
}
...
//
if ( ! function_exists( 'prisma_date_to_sql' ) ) {
	/**
	 ...
	 */
	function prisma_date_to_sql($str) {
		if (trim($str)=='') return '';
		$str = strtr(trim($str),'/\.,','----');
...
if ( ! function_exists( 'prisma_format_price' ) ) {
	/**
	 ...
	 */
	function prisma_format_price( $price ) {
...
if ( ! function_exists( 'prisma_num2size' ) ) {
	/**
	 ...
	 */
	function prisma_num2size( $num ) {
		return $num > 1000 ? round( $num/1000, 0 ) . 'K' : $num;
	}
}

if ( ! function_exists( 'prisma_size2num' ) ) {
	/**
	 ...
	 */
	function prisma_size2num( $size, $full = false ) {
...
//
if ( ! function_exists( 'prisma_parse_num' ) ) {
	/**
...
if ( ! function_exists( 'prisma_prepare_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function prisma_prepare_macros( $str ) {
...
if ( ! function_exists( 'prisma_remove_macros' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function prisma_remove_macros( $str ) {
		return empty( $str ) ? '' : str_replace(
										array( '{{', '}}', '((', '))', '||' ),
										array( '', '', '', '', ' ' ),
										$str
...
if ( ! function_exists( 'prisma_is_on' ) ) {
	/**
...
if ( ! function_exists( 'prisma_is_off' ) ) {
	/**
...
if ( ! function_exists( 'prisma_is_inherit' ) ) {
	/**
	 ...
	 */
	function prisma_is_inherit( $prm ) {
		return ! is_array( $prm ) && in_array( strtolower( $prm ), array( 'inherit' ) );
	}
}

if ( ! function_exists( 'prisma_strshort' ) ) {
	/**
	 ...
	 */
	function prisma_strshort( $str, $maxlength, $add = '&hellip;' ) {
		if ( 0 >= $maxlength ) {
			return '';
		}
		$str = prisma_strip_tags( $str );
...
if ( ! function_exists( 'prisma_strwords' ) ) {
	/**
	 ...
	 * @return string            A truncated string.
	 */
	function prisma_strwords( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		$words = explode( ' ', $str );
...
if ( ! function_exists( 'prisma_strip_tags' ) ) {
	/**
	 ...
	 */
	function prisma_strip_tags( $str ) {
...
if ( ! function_exists( 'prisma_excerpt' ) ) {
	/**
	 ...
	 */
	function prisma_excerpt( $str, $maxlength, $add = '&hellip;' ) {
		if ( $maxlength <= 0 ) {
			return '';
		}
		return prisma_strwords( prisma_strip_tags( $str ), $maxlength, $add );
	}
}

if ( ! function_exists( 'prisma_unserialize_recover' ) ) {
	/**
	 ...
	 * @return string      A processed string.
	 */
	function prisma_unserialize_recover( $str ) {
...
if ( ! function_exists( 'prisma_unserialize' ) ) {
	/**
	 ...
	 */
	function prisma_unserialize( $str, $disable_objects = false ) {
		if ( ! empty( $str ) && is_serialized( $str ) ) {
			...
			if ( ! preg_match( '/O:[0-9]+:"([^"]*)":[0-9]+:{/i', $str, $matches )
				...
				try {
					...
					if ( false === $data ) {
						$data = @unserialize( prisma_unserialize_recover( $str ) );
					}
					// Attempt 4: try unserialize original string without CR symbol '\r' with modified character counters
					if ( false === $data ) {
						$data = @unserialize( prisma_unserialize_recover( $str2 ) );
					}
				} catch ( Exception $e ) {
...
if ( ! function_exists( 'prisma_str_replace' ) ) {
	/**
	 ...
	 */
	function prisma_str_replace( $from, $to, $str ) {
		if ( is_array( $str ) ) {
			foreach ( $str as $k => $v ) {
				$str[ $k ] = prisma_str_replace( $from, $to, $v );
			}
			...
			if ( is_serialized( $str ) ) {
				$str = serialize( prisma_str_replace( $from, $to, prisma_unserialize( $str ) ) );
			} else {
...
if ( ! function_exists( 'prisma_str_replace_once' ) ) {
	/**
	 ...
	 * @return string             A processed string.
	 */
	function prisma_str_replace_once( $from, $to, $str ) {
		$rez = '';
		if ( ! is_array( $from ) ) {
			$from = array( $from );
		}
...
if ( ! function_exists( 'prisma_tags_count' ) ) {
	/**
	 ...
	 */
	function prisma_tags_count( $str, $tag ) {
		$cnt = 0;
...
if ( ! function_exists( 'prisma_getimagesize' ) ) {
	/**
	 ...
	 */
	function prisma_getimagesize( $url, $echo = false ) {
		$img_size = false;
		if ( is_string( $url ) ) {
			$img_path = prisma_is_url( $url ) ? prisma_url_to_local_path( $url ) : $url;
...
if ( ! function_exists( 'prisma_clear_thumb_size' ) ) {
	/**
	 ...
	 */
	function prisma_clear_thumb_size( $url, $remove_protocol = true ) {
		if ( empty( $url ) ) return '';
		$pi = pathinfo( $url );
		if ( $remove_protocol ) {
			$pi['dirname'] = prisma_remove_protocol_from_url( $pi['dirname'], false );
		}
...
if ( ! function_exists( 'prisma_add_thumb_size' ) ) {
	/**
	 ...
	 */
	function prisma_add_thumb_size( $url, $thumb_size, $check_exists = true ) {
		...
		$new_url = prisma_remove_protocol_from_url( ( ! empty( $pi['dirname'] ) ? $pi['dirname'] . '/' : '' ) . $pi['filename'] . ( ! empty( $pi['extension'] ) ? '.' . $pi['extension'] : '' ) );

		// Check exists
		if ( $check_exists ) {
			$uploads_info = wp_upload_dir();
			$uploads_url = prisma_remove_protocol_from_url( $uploads_info['baseurl'] );
			...
			if ( strpos( $new_url, $uploads_url ) !== false ) {
				if ( ! file_exists( str_replace( $uploads_url, $uploads_dir, $new_url ) ) ) {
					$new_url = prisma_remove_protocol_from_url( $url );
				}
			} else {
				$new_url = prisma_remove_protocol_from_url( $url );
			}
...
if ( ! function_exists( 'prisma_get_thumb_size' ) ) {
	 ...
	 */
	function prisma_get_thumb_size( $ts ) {
		$thumb_sizes = prisma_storage_get( 'theme_thumbs' );
		if ( ! empty( $thumb_sizes[ "prisma-thumb-{$ts}" ] ) ) {
			$ts = 'prisma-thumb-' . $ts;
		}
		$retina = prisma_get_retina_multiplier() > 1 ? '-@retina' : '';
		$is_external = apply_filters(
							'prisma_filter_is_external_thumb_size',
							in_array( $ts,
										apply_filters(
											'prisma_filter_external_thumb_sizes',
		...
		$is_internal = strpos( $ts, 'prisma-thumb-' ) === 0 || strpos( $ts, 'trx_addons-thumb-' ) === 0;
		return apply_filters(
					'prisma_filter_get_thumb_size',
					( $is_external || $is_internal ? '' : 'prisma-thumb-' )
					. $ts
					. ( $is_internal ? $retina : '' )
				);
	}
...
if ( ! function_exists( 'prisma_get_closest_thumb_size' ) ) {
	 ...
	 */
	function prisma_get_closest_thumb_size( $old_size, $dim, $prefix = '' ) {
...
if ( ! function_exists( 'prisma_get_attachment_url' ) ) {
	/**
	 ...
	 * @return string
	 */
	function prisma_get_attachment_url( $image_id, $size = 'full' ) {
		...
		if ( is_numeric( $image_id ) && (int) $image_id > 0 ) {
			...
			$image_id = prisma_add_thumb_size( $image_id, $size );
		}
		return $image_id;
	}
}

if ( ! function_exists( 'prisma_get_post_image' ) ) {
	/**
	 ...
	 */
	function prisma_get_post_image( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		if ( empty( $post_text ) ) {
...
if ( ! function_exists( 'prisma_get_post_audio' ) ) {
	/**
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function prisma_get_post_audio( $post_text = '', $src = true ) {
		global $post;
		$img = prisma_get_post_audio_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['url'] ) ) {
				$img = $img['url'];
			} else if ( ! empty( $img['embed'] ) ) {
				$img = prisma_get_post_iframe( $img['embed'], true );
			} else {
				$img = '';
			}
		}
		if ( empty( $img ) ) {
			...
			if ( $src ) {
				...
				$img = prisma_get_tag( $post_text, '<audio', '</audio>' );
				if ( empty( $img ) ) {
					$img = do_shortcode( prisma_get_tag( $post_text, '[audio', '[/audio]' ) );
				}
				if ( empty( $img ) ) {
					$img = prisma_get_tag_attrib( $post_text, '[trx_widget_audio]', 'url' );
...
if ( ! function_exists( 'prisma_get_post_audio_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function prisma_get_post_audio_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_audio_list' ) ? trx_addons_get_post_audio_list( $args ) : array();
	}
}

if ( ! function_exists( 'prisma_get_post_audio_list_first' ) ) {
	/**
...
if ( ! function_exists( 'prisma_get_post_video' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function prisma_get_post_video( $post_text = '', $src = true ) {
		global $post;
		$img = prisma_get_post_video_list_first( ! $src );
		if ( is_array( $img ) ) {
			if ( ! empty( $img['video_url'] ) ) {
				$img = $img['video_url'];
			} else if ( ! empty( $img['video_embed'] ) ) {
				$img = prisma_get_post_iframe( $img['video_embed'], true );
			} else {
				$img = '';
			}
		}
		...
		return apply_filters( 'prisma_filter_get_post_video', $img );
	}
}

if ( ! function_exists( 'prisma_get_post_video_list' ) ) {
	/**
	 ...
	 *                     ]
	 */
	function prisma_get_post_video_list( $args=array() ) {
		return function_exists( 'trx_addons_get_post_video_list' ) ? trx_addons_get_post_video_list( $args ) : array();
	}
}

if ( ! function_exists( 'prisma_get_post_video_list_first' ) ) {
	/**
...
if ( ! function_exists( 'prisma_get_post_iframe' ) ) {
	 ...
	 * @return string           Founded URL (or a whole tag) or an empty string if found nothing.
	 */
	function prisma_get_post_iframe( $post_text = '', $src = true ) {
		global $post;
		$img = '';
		$tag = prisma_get_embed_video_tag_name();
		if ( empty( $post_text ) ) {
			$post_text = $post->post_content;
		}
		if ( $src ) {
			...
			$img = prisma_get_tag( $post_text, '<' . esc_html( $tag ), '</' . esc_html( $tag ) . '>' );
		}
		return apply_filters( 'prisma_filter_get_post_iframe', $img );
	}
}

if ( ! function_exists( 'prisma_get_embed_video' ) ) {
	/**
	...
	function prisma_get_embed_video( $video, $use_wp_embed = false, $args = array() ) {
		...
		if ( $use_wp_embed && is_object( $wp_embed ) ) {
			...
			if ( strpos( $video, 'youtu.be/' ) !== false || strpos( $video, 'youtube.com/' ) !== false ) {
				...
				$video = prisma_add_to_url( $video, prisma_get_embed_video_parameters() );
			}
			...
			$ratio  = apply_filters( 'prisma_filter_video_ratio', $is_tiktok ? '9:21' : '16:9' );
			$parts  = explode( ':', $ratio );
			$width  = $is_tiktok ? apply_filters( 'prisma_filter_video_width', 325, 'tiktok' ) : prisma_get_content_width();
			$height = round( $width / $parts[0] * $parts[1] );
			$dim    = apply_filters( 'prisma_filter_video_dimensions', array(
									'width' => $width,
									'height' => $height
									), $ratio );
...
if ( ! function_exists( 'prisma_get_embed_video_tag_name' ) ) {
	/**
	 * Return a name of the tag-container to embed video.
	 *
	 * @return string  A tag name to embed video.
	 */
	function prisma_get_embed_video_tag_name() {
...
if ( ! function_exists( 'prisma_make_video_autoplay' ) ) {
	/**
	 ...
	 */
	function prisma_make_video_autoplay( $video, $muted = false ) {
					...
					if ( ! empty( $matches[2] ) ) {
						...
						$matches[2] = prisma_add_to_url(
							...
							array_merge(
								...
								$muted
									? array_merge(
										prisma_get_embed_video_parameters(),
										array(
											'muted' => 1,
											'controls' => 0,
											'background' => 1,
...
if ( ! function_exists( 'prisma_is_from_uploads' ) ) {
	/**
	 ...
	 */
	function prisma_is_from_uploads( $url ) {
		$url          = prisma_remove_protocol_from_url( $url );
		$parts        = explode( '?', $url );
		$url          = $parts[0];
		$uploads_info = wp_upload_dir();
		$uploads_url  = prisma_remove_protocol_from_url( $uploads_info['baseurl'] );
...
if ( ! function_exists( 'prisma_is_youtube_url' ) ) {
	/**
	 ...
	 */
	function prisma_is_youtube_url( $url ) {
		return strpos( $url, 'youtu.be' ) !== false || strpos( $url, 'youtube.com' ) !== false;
	}
}

if ( ! function_exists( 'prisma_is_vimeo_url' ) ) {
	/**
	 ...
	 */
	function prisma_is_vimeo_url( $url ) {
...
if ( ! function_exists( 'prisma_mkdir' ) ) {
	/**
...
if ( ! function_exists( 'prisma_unlink' ) ) {
	/**
	 ...
	 */
	function prisma_unlink( $path, $recursive = true, $type = 'd' ) {
		if ( ! empty( $path ) ) {
...
if ( ! function_exists( 'prisma_copy' ) ) {
	/**
...
if ( ! function_exists( 'prisma_unzip_file' ) ) {
	/**
	 ...
	 * @return bool         Return true if no error occurs.
	 */
	function prisma_unzip_file( $zip, $dest ) {
...
if ( ! function_exists( 'prisma_retrieve_json' ) ) {
	/**
...
if ( ! function_exists( 'prisma_esc' ) ) {
	 ...
	 */
	function prisma_esc( $name ) {
		return str_replace( array( '\\', '~', '$', ':', ';', '+', '>', '<', '|', '"', "'", '`', "\xFF", "\x0A", "\x0D", '*', '?', '^' ), '/', trim( $name ) );
	}
}

if ( ! function_exists( 'prisma_prepare_path' ) ) {
	/**
...
if ( ! function_exists( 'prisma_check_min_file' ) ) {
	 ...
	 */
	function prisma_check_min_file( $file, $dir = '' ) {
		if ( empty( $dir ) ) {
			$dir = dirname( $file );
		}
		if ( substr( $file, -3 ) == '.js' ) {
			if ( substr( $file, -7 ) != '.min.js' && prisma_is_off( prisma_get_theme_option( 'debug_mode' ) ) ) {
			...
			if ( substr( $file, -8 ) != '.min.css' && prisma_is_off( prisma_get_theme_option( 'debug_mode' ) ) ) {
				$dir      = trailingslashit( $dir );
...
if ( ! function_exists( 'prisma_get_file_dir' ) ) {
	/**
	 ...
	 */
	function prisma_get_file_dir( $file, $return_url = false ) {
		// Use new WordPress functions (if present)
		if ( function_exists( 'get_theme_file_path' ) && ! PRISMA_ALLOW_SKINS && ! prisma_get_theme_setting( 'check_min_version', false ) ) {
		...
		// Otherwise (on WordPress older then 4.7.0) or theme use .min versions of .js and .css or theme use skins
			...
			$dir       = '';
			$theme_dir = apply_filters( 'prisma_filter_get_theme_file_dir', '', $file, $return_url );
			if ( '' != $theme_dir ) {
				$dir = $theme_dir;
			} elseif ( PRISMA_CHILD_DIR != PRISMA_THEME_DIR && file_exists( PRISMA_CHILD_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? PRISMA_CHILD_URL : PRISMA_CHILD_DIR ) . prisma_check_min_file( $file, PRISMA_CHILD_DIR );
			} elseif ( file_exists( PRISMA_THEME_DIR . ( $file ) ) ) {
				$dir = ( $return_url ? PRISMA_THEME_URL : PRISMA_THEME_DIR ) . prisma_check_min_file( $file, PRISMA_THEME_DIR );
			}
		}
		return $dir;
	}
...
if ( ! function_exists( 'prisma_get_file_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a file is not exists - return an empty string.
	 */
	function prisma_get_file_url( $file ) {
		return prisma_get_file_dir( $file, true );
	}
}

if ( ! function_exists( 'prisma_get_file_ext' ) ) {
	/**
	 ...
	 */
	function prisma_get_file_ext( $file ) {
		$ext = pathinfo( $file, PATHINFO_EXTENSION );
		return empty( $ext ) ? '' : $ext;
	}
}

if ( ! function_exists( 'prisma_get_file_name' ) ) {
	/**
	 ...
	 */
	function prisma_get_file_name( $file, $without_ext = true ) {
		$parts = pathinfo( $file );
		return ! empty( $parts['filename'] ) && $without_ext ? $parts['filename'] : $parts['basename'];
	}
}

if ( ! function_exists( 'prisma_get_folder_dir' ) ) {
	/**
	 ...
	 */
	function prisma_get_folder_dir( $folder, $return_url = false ) {
		if ( '/' == $folder[0] ) {
			$folder = substr( $folder, 1 );
		}
		$dir       = '';
		$theme_dir = apply_filters( 'prisma_filter_get_theme_folder_dir', '', $folder, $return_url );
		if ( '' != $theme_dir ) {
			$dir = $theme_dir;
		} elseif ( PRISMA_CHILD_DIR != PRISMA_THEME_DIR && is_dir( PRISMA_CHILD_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? PRISMA_CHILD_URL : PRISMA_CHILD_DIR ) . ( $folder );
		} elseif ( is_dir( PRISMA_THEME_DIR . ( $folder ) ) ) {
			$dir = ( $return_url ? PRISMA_THEME_URL : PRISMA_THEME_DIR ) . ( $folder );
		}
		return apply_filters( 'prisma_filter_get_folder_dir', $dir, $folder, $return_url );
	}
}

if ( ! function_exists( 'prisma_get_folder_url' ) ) {
	/**
	 ...
	 *                          or in the main theme directory. If a folder is not exists - return an empty string.
	 */
	function prisma_get_folder_url( $folder ) {
		return prisma_get_folder_dir( $folder, true );
	}
}

if ( ! function_exists( 'prisma_merge_js' ) ) {
	/**
	 ...
	 */
	function prisma_merge_js( $to, $list ) {
		$s = '';
		foreach ( $list as $f ) {
			$s .= prisma_fgc( prisma_get_file_dir( $f ) );
		}
		if ( '' != $s ) {
			$file_dir = prisma_get_file_dir( $to );
			if ( empty( $file_dir ) && strpos( $to, '-full.js' ) !== false ) {
				$file_dir = prisma_get_file_dir( str_replace( '-full.js', '.js', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			prisma_fpc( $file_dir,
				'/* '
				. strip_tags( __( "ATTENTION! This file was generated automatically! Don't change it!!!", 'prisma' ) )
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'prisma_filter_js_output', apply_filters( 'prisma_filter_prepare_js', $s, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'prisma_merge_css' ) ) {
	/**
	 ...
	 */
	function prisma_merge_css( $to, $list, $need_responsive = false ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'prisma_filter_responsive_sizes', prisma_storage_get( 'responsive' ) );
		}
		$sizes  = array();
		$output = '';
		foreach ( $list as $f ) {
			$fdir = prisma_get_file_dir( $f );
			if ( '' != $fdir ) {
				$css = prisma_fgc( $fdir );
				if ( $need_responsive ) {
					$pos = 0;
					while( false !== $pos ) {
						...
						if ( false !== $pos ) {
							...
							$sizes[ $media ] .= "\n\n" . apply_filters( 'prisma_filter_merge_css', substr( $css, $pos_lbrace + 1, $pos_rbrace - $pos_lbrace - 1 ) );
							$css = substr( $css, $pos_rbrace + 1);
						}
					}
				} else {
					$output .= "\n\n" . apply_filters( 'prisma_filter_merge_css', $css );
				}
			}
		}
		if ( $need_responsive ) {
			foreach ( $responsive as $k => $v ) {
				...
				if ( ! empty( $sizes[ $media ] ) ) {
					$output .= "\n\n"
							...
							. strip_tags( sprintf( __( '/* SASS Suffix: --%s */', 'prisma' ), $k ) )
							. "\n"
							. "@media {$media} {\n"
								. $sizes[ $media ]
							. "\n}\n";
			...
			if ( count( $sizes ) > 0 ) {
				$output .= "\n\n"
						. strip_tags( __( '/* Unknown Suffixes: */', 'prisma' ) );
		...
		if ( '' != $output ) {
			...
			if ( empty( $file_dir ) && strpos( $to, '-full.css' ) !== false ) {
				$file_dir = prisma_get_file_dir( str_replace( '-full.css', '.css', $to ) );
				if ( ! empty( $file_dir ) ) {
			...
			prisma_fpc( $file_dir,
				...
				. strip_tags( __("ATTENTION! This file was generated automatically! Don't change it!!!", 'prisma') ) 
				. "\n----------------------------------------------------------------------- */\n"
				. apply_filters( 'prisma_filter_css_output', apply_filters( 'prisma_filter_prepare_css', $output, true ), $to )
			);
		}
	}
}

if ( ! function_exists( 'prisma_filter_merge_css' ) ) {
	add_filter( 'prisma_filter_merge_css', 'prisma_filter_merge_css' );
	/**
	 * Replace all relative paths in the merged CSS-file to resolve url() attributes.
	 *
	 * @hooked 'prisma_filter_merge_css'
	 *
	 * @param string $css  A merged CSS content to resolve paths.
	 *
	 * @return string      A processed CSS content
	 */
	function prisma_filter_merge_css( $css ) {
		return str_replace( '../../../../', '../../../', $css);
	}
}

if ( ! function_exists( 'prisma_merge_sass' ) ) {
	/**
	 ...
	 */
	function prisma_merge_sass( $to, $list, $need_responsive = false, $root = '../' ) {
		if ( $need_responsive ) {
			$responsive = apply_filters( 'prisma_filter_responsive_sizes', prisma_storage_get( 'responsive' ) );
		}
		$sass                = array(
		...
		foreach ( $list as $f ) {
			...
			if ( $add ) {
				$sass['import'] .= apply_filters( 'prisma_filter_sass_import', "{$sass_import} \"{$root}{$f}\";\n", $f );
				$save            = true;
			}
		}
		if ( $save ) {
			...
			prisma_fpc( prisma_get_file_dir( $to ), apply_filters( 'prisma_filter_sass_output', $output, $to ) );
		}
	}
}
...
if ( ! function_exists( 'prisma_get_domain_from_url' ) ) {
	/**
	 ...
	 */
	function prisma_get_domain_from_url( $url ) {
		$pos = strpos( $url, '//' );
		if ( $pos !== false ) {
...
if ( ! function_exists( 'prisma_add_protocol' ) ) {
	/**
	 ...
	 */
	function prisma_add_protocol( $url ) {
		return substr( $url, 0, 2 ) == '//' ? prisma_get_protocol() . ':' . $url : $url;
	}
}

if ( ! function_exists( 'prisma_remove_protocol_from_url' ) ) {
	/**
	 ...
	 * @return string         A processed string.
	 */
	function prisma_remove_protocol_from_url( $url, $complete = true ) {
		return preg_replace( '/(http[s]?:)?' . ( $complete ? '\\/\\/' : '' ) . '/', '', $url );
	}
}

if ( ! function_exists( 'prisma_add_to_url' ) ) {
	/**
	 ...
	 */
	function prisma_add_to_url( $url, $prm ) {
		if ( is_array( $prm ) && count( $prm ) > 0 ) {
			...
			$params = prisma_array_merge( $params, $prm );
			$url = $parts[0];
			$separator = '?';
...
if ( ! function_exists( 'prisma_is_url' ) ) {
	/**
	 ...
	 */
	function prisma_is_url( $url ) {
		return is_string( $url ) && ( strpos( $url, '//' ) === 0 || strpos( $url, '://' ) !== false );
	}
}

if ( ! function_exists( 'prisma_is_external_url' ) ) {
	/**
	 ...
	 */
	function prisma_is_external_url( $url ) {
		...
		return prisma_is_url( $url ) && strpos( $url, prisma_remove_protocol_from_url( $parts[0], true ) ) === false;
	}
}

if ( ! function_exists( 'prisma_url_to_local_path' ) ) {
	/**
	 ...
	 */
	function prisma_url_to_local_path( $url ) {
		$path = '';
		...
		$locations = array(
			'uploads' => array(
				...
				'url' => prisma_remove_protocol_from_url( $upload_info['baseurl'] )
				),
			'child' => array(
				'dir' => PRISMA_CHILD_DIR,
				'url' => prisma_remove_protocol_from_url( PRISMA_CHILD_URL )
				),
			'theme' => array(
				'dir' => PRISMA_THEME_DIR,
				'url' => prisma_remove_protocol_from_url( PRISMA_THEME_URL )
				)
			);
