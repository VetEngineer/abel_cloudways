...
if ( ! function_exists( 'prisma_storage_get' ) ) {
	/**
	 ...
	 */
	function prisma_storage_get( $var_name, $default = '' ) {
		global $PRISMA_STORAGE;
		return isset( $PRISMA_STORAGE[ $var_name ] ) ? $PRISMA_STORAGE[ $var_name ] : $default;
	}
}

if ( ! function_exists( 'prisma_storage_set' ) ) {
	/**
	 ...
	 */
	function prisma_storage_set( $var_name, $value ) {
		global $PRISMA_STORAGE;
		$PRISMA_STORAGE[ $var_name ] = $value;
	}
}

if ( ! function_exists( 'prisma_storage_empty' ) ) {
	/**
	 ...
	 */
	function prisma_storage_empty( $var_name, $key = '', $key2 = '' ) {
		global $PRISMA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return empty( $PRISMA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return empty( $PRISMA_STORAGE[ $var_name ][ $key ] );
		} else {
			return empty( $PRISMA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'prisma_storage_isset' ) ) {
	/**
	 ...
	 */
	function prisma_storage_isset( $var_name, $key = '', $key2 = '' ) {
		global $PRISMA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			return isset( $PRISMA_STORAGE[ $var_name ][ $key ][ $key2 ] );
		} elseif ( '' !== $key ) {
			return isset( $PRISMA_STORAGE[ $var_name ][ $key ] );
		} else {
			return isset( $PRISMA_STORAGE[ $var_name ] );
		}
	}
}

if ( ! function_exists( 'prisma_storage_unset' ) ) {
	/**
	 ...
	 */
	function prisma_storage_unset( $var_name, $key = '', $key2 = '' ) {
		global $PRISMA_STORAGE;
		if ( '' !== $key && '' !== $key2 ) {
			if ( isset( $PRISMA_STORAGE[ $var_name ][ $key ][ $key2 ] ) ) {
				unset( $PRISMA_STORAGE[ $var_name ][ $key ][ $key2 ] );
			}
		} elseif ( '' !== $key ) {
			if ( isset( $PRISMA_STORAGE[ $var_name ][ $key ] ) ) {
				unset( $PRISMA_STORAGE[ $var_name ][ $key ] );
			}
		} else {
			if ( isset( $PRISMA_STORAGE[ $var_name ] ) ) {
				unset( $PRISMA_STORAGE[ $var_name ] );
			}
		}
	}
}

if ( ! function_exists( 'prisma_storage_inc' ) ) {
	/**
	 ...
	 */
	function prisma_storage_inc( $var_name, $value = 1 ) {
		global $PRISMA_STORAGE;
		if ( empty( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = 0;
		}
		$PRISMA_STORAGE[ $var_name ] += $value;
	}
}

if ( ! function_exists( 'prisma_storage_concat' ) ) {
	/**
	 ...
	 */
	function prisma_storage_concat( $var_name, $value ) {
		global $PRISMA_STORAGE;
		if ( empty( $PRISMA_STORAGE[ $var_name ] ) ) {
...
if ( ! function_exists( 'prisma_storage_get_array' ) ) {
	/**
	 ...
	 */
	function prisma_storage_get_array( $var_name, $key, $key2 = '', $default = '' ) {
		global $PRISMA_STORAGE;
		if ( '' === $key2 ) {
			return ! empty( $var_name ) && '' !== $key && isset( $PRISMA_STORAGE[ $var_name ][ $key ] ) ? $PRISMA_STORAGE[ $var_name ][ $key ] : $default;
		} else {
			return ! empty( $var_name ) && '' !== $key && isset( $PRISMA_STORAGE[ $var_name ][ $key ][ $key2 ] ) ? $PRISMA_STORAGE[ $var_name ][ $key ][ $key2 ] : $default;
		}
	}
}

if ( ! function_exists( 'prisma_storage_set_array' ) ) {
	/**
	 ...
	 */
	function prisma_storage_set_array( $var_name, $key, $value ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $PRISMA_STORAGE[ $var_name ] ) ) {
			if ( '' === $key ) {
				$PRISMA_STORAGE[ $var_name ][] = $value;
			} else {
...
if ( ! function_exists( 'prisma_storage_set_array2' ) ) {
	/**
	 ...
	 * @param mixed  $value     A new value for the specified element of the array.
	 */
	function prisma_storage_set_array2( $var_name, $key, $key2, $value ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( ! isset( $PRISMA_STORAGE[ $var_name ][ $key ] ) ) {
			$PRISMA_STORAGE[ $var_name ][ $key ] = array();
		}
		if ( is_array( $PRISMA_STORAGE[ $var_name ][ $key ] ) ) {
			if ( '' === $key2 ) {
				$PRISMA_STORAGE[ $var_name ][ $key ][] = $value;
			} else {
				$PRISMA_STORAGE[ $var_name ][ $key ][ $key2 ] = $value;
			}
		}
	}
}

if ( ! function_exists( 'prisma_storage_merge_array' ) ) {
	/**
	 ...
	 */
	function prisma_storage_merge_array( $var_name, $key, $value ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			$PRISMA_STORAGE[ $var_name ] = array_merge( $PRISMA_STORAGE[ $var_name ], $value );
		} else {
			$PRISMA_STORAGE[ $var_name ][ $key ] = array_merge( $PRISMA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'prisma_storage_set_array_after' ) ) {
	/**
	 ...
	 */
	function prisma_storage_set_array_after( $var_name, $after, $key, $value = '' ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			prisma_array_insert_after( $PRISMA_STORAGE[ $var_name ], $after, $key );
		} else {
			prisma_array_insert_after( $PRISMA_STORAGE[ $var_name ], $after, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'prisma_storage_set_array_before' ) ) {
	/**
	 * Insert new elements to the array in the global theme storage
	 ...
	 * @param mixed        $value     A new value to insert (if the argument $key is a string).
	 */
	function prisma_storage_set_array_before( $var_name, $before, $key, $value = '' ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( is_array( $key ) ) {
			prisma_array_insert_before( $PRISMA_STORAGE[ $var_name ], $before, $key );
		} else {
			prisma_array_insert_before( $PRISMA_STORAGE[ $var_name ], $before, array( $key => $value ) );
		}
	}
}

if ( ! function_exists( 'prisma_storage_push_array' ) ) {
	/**
	 ...
	 */
	function prisma_storage_push_array( $var_name, $key, $value ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( '' === $key ) {
			array_push( $PRISMA_STORAGE[ $var_name ], $value );
		} else {
			if ( ! isset( $PRISMA_STORAGE[ $var_name ][ $key ] ) ) {
				$PRISMA_STORAGE[ $var_name ][ $key ] = array();
			}
			array_push( $PRISMA_STORAGE[ $var_name ][ $key ], $value );
		}
	}
}

if ( ! function_exists( 'prisma_storage_pop_array' ) ) {
	/**
	 ...
	 */
	function prisma_storage_pop_array( $var_name, $key = '', $defa = '' ) {
		global $PRISMA_STORAGE;
		$rez = $defa;
		if ( '' === $key ) {
			if ( isset( $PRISMA_STORAGE[ $var_name ] ) && is_array( $PRISMA_STORAGE[ $var_name ] ) && count( $PRISMA_STORAGE[ $var_name ] ) > 0 ) {
				$rez = array_pop( $PRISMA_STORAGE[ $var_name ] );
			}
		} else {
			if ( isset( $PRISMA_STORAGE[ $var_name ][ $key ] ) && is_array( $PRISMA_STORAGE[ $var_name ][ $key ] ) && count( $PRISMA_STORAGE[ $var_name ][ $key ] ) > 0 ) {
				$rez = array_pop( $PRISMA_STORAGE[ $var_name ][ $key ] );
			}
		}
		return $rez;
	}
...
if ( ! function_exists( 'prisma_storage_inc_array' ) ) {
	/**
	 ...
	 * @param int          $value     Optional. A value for increment. Default is 1. Specify -1 to decrement or any legal number value.
	 */
	function prisma_storage_inc_array( $var_name, $key, $value = 1 ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $PRISMA_STORAGE[ $var_name ][ $key ] ) ) {
			$PRISMA_STORAGE[ $var_name ][ $key ] = 0;
		}
		$PRISMA_STORAGE[ $var_name ][ $key ] += $value;
	}
}

if ( ! function_exists( 'prisma_storage_concat_array' ) ) {
	/**
	 ...
	 * @param int          $value     A value to concatenate.
	 */
	function prisma_storage_concat_array( $var_name, $key, $value ) {
		global $PRISMA_STORAGE;
		if ( ! isset( $PRISMA_STORAGE[ $var_name ] ) ) {
			$PRISMA_STORAGE[ $var_name ] = array();
		}
		if ( empty( $PRISMA_STORAGE[ $var_name ][ $key ] ) ) {
			$PRISMA_STORAGE[ $var_name ][ $key ] = '';
		}
		$PRISMA_STORAGE[ $var_name ][ $key ] .= $value;
	}
}

if ( ! function_exists( 'prisma_storage_call_obj_method' ) ) {
	/**
	 ...
	 */
	function prisma_storage_call_obj_method( $var_name, $method, $param = null ) {
		global $PRISMA_STORAGE;
		if ( null === $param ) {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PRISMA_STORAGE[ $var_name ] ) ? $PRISMA_STORAGE[ $var_name ]->$method() : '';
		} else {
			return ! empty( $var_name ) && ! empty( $method ) && isset( $PRISMA_STORAGE[ $var_name ] ) ? $PRISMA_STORAGE[ $var_name ]->$method( $param ) : '';
		}
	}
}

if ( ! function_exists( 'prisma_storage_get_obj_property' ) ) {
	/**
	 ...
	 */
	function prisma_storage_get_obj_property( $var_name, $prop, $default = '' ) {
		global $PRISMA_STORAGE;
		return ! empty( $var_name ) && ! empty( $prop ) && isset( $PRISMA_STORAGE[ $var_name ]->$prop ) ? $PRISMA_STORAGE[ $var_name ]->$prop : $default;
	}
}
