...
if ( ! defined( 'PRISMA_THEME_FREE' ) ) {
	define( 'PRISMA_THEME_FREE', false );
}
if ( ! defined( 'PRISMA_THEME_FREE_WP' ) ) {
	define( 'PRISMA_THEME_FREE_WP', false );
}

// If this theme support external updates
if ( ! defined( 'PRISMA_THEME_ALLOW_UPDATE' ) ) {
	define( 'PRISMA_THEME_ALLOW_UPDATE', true );
}

// If this theme uses skins
if ( ! defined( 'PRISMA_ALLOW_SKINS' ) ) {
	define( 'PRISMA_ALLOW_SKINS', true );
}
if ( ! defined( 'PRISMA_DEFAULT_SKIN' ) ) {
	define( 'PRISMA_DEFAULT_SKIN', 'default' );
}
if ( ! defined( 'PRISMA_REMEMBER_SKIN' ) ) {
	define( 'PRISMA_REMEMBER_SKIN', false );
}
...
if ( ! function_exists( 'prisma_get_theme_pro_key' ) ) {
	/**
	 ...
	 */
	function prisma_get_theme_pro_key() {
		$theme_pro_key = get_option( sprintf( 'purchase_code_src_%s', get_template() ) );
		if ( empty( $theme_pro_key ) ) {
			$theme_pro_key = prisma_storage_get( 'theme_pro_key' );
		}
		return $theme_pro_key;
	}
}

if ( ! function_exists( 'prisma_trx_updater_get_theme_info' ) ) {
	add_filter( 'trx_updater_filter_get_theme_info', 'prisma_trx_updater_get_theme_info' );
	/**
	 ...
	 */
	function prisma_trx_updater_get_theme_info( $info ) {
		$engine = prisma_storage_get( 'engine' );
...
if ( ! function_exists( 'prisma_original_theme_slug' ) ) {
	add_filter( 'prisma_filter_original_theme_slug', 'prisma_original_theme_slug' );
	/**
...
if ( ! function_exists( 'prisma_importer_set_options' ) ) {
	add_filter( 'trx_addons_filter_importer_options', 'prisma_importer_set_options', 9 );
	/**
	 ...
	 */
	function prisma_importer_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			...
			// Prepare demo data
			$options['demo_url'] = esc_url( prisma_get_protocol() . ':' . prisma_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
			$options['required_plugins'] = array_keys( prisma_storage_get( 'required_plugins' ) );
...
if ( ! function_exists( 'prisma_ocdi_set_options' ) ) {
	add_filter( 'trx_addons_filter_ocdi_options', 'prisma_ocdi_set_options', 9 );
	/**
	 ...
	 * @return array          Processed options with a theme-specific values.
	 */
	function prisma_ocdi_set_options( $options = array() ) {
		if ( is_array( $options ) ) {
			// Prepare demo data
			$options['demo_url'] = esc_url( prisma_get_protocol() . ':' . prisma_storage_get( 'theme_demofiles_url' ) );
			// Required plugins
...
if ( ! function_exists( 'prisma_customizer_theme_setup1' ) ) {
	add_action( 'after_setup_theme', 'prisma_customizer_theme_setup1', 1 );
	/**
	 ...
	 */
	function prisma_customizer_theme_setup1() {
		prisma_storage_set(
			'settings', array(
				...
				'allow_fullscreen'          => false,                   // Allow cases 'fullscreen' and 'fullwide' (if a next settings is true) for the body style in the Theme Options
																		...
																		// (can be removed if filter 'prisma_filter_allow_fullscreen' return false)
