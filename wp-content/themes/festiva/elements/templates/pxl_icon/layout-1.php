...
if(isset($settings['icons']) && !empty($settings['icons']) && count($settings['icons'])): ?>
    <div class="pxl-icon-list pxl-icon1 <?php echo esc_attr($settings['animation_icon']); ?> <?php echo esc_attr('pxl-type-'.$settings['box_color_hover_type'].' '.$settings['style'].' '.$settings['pxl_animate']); ?>" data-wow-delay="<?php echo esc_attr($settings['pxl_animate_delay']); ?>ms"> 
        ...
        <?php foreach ($settings['icons'] as $key => $value):
            ...
            <?php if ( ! empty( $value['pxl_icon'] ) ) : ?>
                <a class="elementor-repeater-item-<?php echo esc_attr($value['_id']); ?> " <?php echo implode( ' ', [ $link_attributes ] ); ?>>
                    ...
                    <?php endif; ?>
                <span class="sub_text"><?php echo pxl_print_html($value['sub_text']) ?></span>
                </a>
            <?php endif; ?>
        <?php endforeach; ?>
    </div>
