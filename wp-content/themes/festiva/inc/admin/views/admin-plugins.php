...
$status_ins = false;
$status_act = false;
$btn_text = esc_html__('Install All', 'festiva');
...
$merlin_setup = get_option( 'merlin_' . festiva()->get_slug() . '_completed' );
 
if( $status_ins && $status_act)
	$btn_text = esc_html__('Install & Active All', 'festiva');
else if($status_ins && !$status_act)
	$btn_text = esc_html__('Install All', 'festiva');
else if(!$status_ins && $status_act)
	$btn_text = esc_html__('Active All', 'festiva');
?>
<main>
	...
	<div class="pxl-dashboard-wrap">
		...
		<header class="pxl-dsb-header admin-plugin">
			<div class="pxl-dsb-header-inner">
				<h4><?php esc_html_e( 'Install Plugins', 'festiva' ); ?></h4>
			...
			<p><?php esc_html_e( 'Make sure to activate required plugins prior to import a demo.', 'festiva' ); ?></p> 
		</header>
	        ...
	        <?php
				...
				foreach( $plugins as $plugin ) :
					...
					elseif( !is_plugin_inactive( $file_path ) ) {
						...
						$display_status = esc_html__( 'Active:', 'festiva' );
					}
			?>
				<div class="pxl-col pxl-col-3">
					...
					<?php 
					$barplugin = new Festiva_Admin_Plugins;
