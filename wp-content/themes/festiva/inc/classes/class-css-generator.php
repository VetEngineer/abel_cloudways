...
class Festiva_CSS_Generator {
	...
	function __construct() {
		$this->opt_name = festiva()->get_option_name();  
        ...
        add_action( 'wp_enqueue_scripts', array( $this, 'festiva_enqueue' ), 20 );
	}
	...
	function init() {
		...
		add_action( 'wp', array( $this, 'festiva_generate_with_dev_mode' ) );
		add_action( "redux/options/{$this->opt_name}/saved", function () {
			$this->festiva_generate_file_options();
		} );
	}

	function festiva_generate_with_dev_mode() {
		if ( $this->dev_mode === true ) {
            $this->festiva_generate_file_options();
			$this->festiva_generate_file();
		}
	}

    function festiva_generate_file_options() {
        ...
        $this->redux->filesystem->execute( 'put_contents', $_options, array(
            'content' => preg_replace( "/(?<=[^\r]|^)\n/", "\r\n", $this->festiva_options_output() )
        ) );
    }

	function festiva_generate_file() {
		$scss_dir = get_template_directory() . '/assets/scss/';
	...
	protected function festiva_options_output() {
		$theme_colors                    = festiva_configs('theme_colors');
        $links                           = festiva_configs('link');
        $gradients                       = festiva_configs('gradient');
		ob_start();

		printf('$festiva_theme_colors_key:(%s);',$this->print_scss_opt_colors($theme_colors,'key'));
        printf('$festiva_theme_colors_val:(%s);',$this->print_scss_opt_colors($theme_colors,'val'));
        // color rgb only
        ...
        if(isset($theme_default['font-family'])) {
            if($theme_default['font-family'] == false) {
                echo '
                    $ft_theme_default: "Libre Baskerville";
                ';
            } else {
                echo '
    ...
    /* Inline CSS */
    function festiva_enqueue() {
        $css = $this->festiva_inline_css();
        if ( !empty( $css ) ) {
            wp_add_inline_style( 'pxl-style', $css );
        }
    }
    protected function festiva_inline_css() {
        ob_start();
